...
try:
    ...
    print(
        "LightDiffusion not available or incorrectly accessed, proceeding without generative AI"
    )
...
async def status_task():
    ...
    if current_state == "Deadge":
        status_task.cancel()
        return
    current_state = random.choices(states, weights=probabilities, k=1)[0]
    await bot.change_presence(
        status=discord.Status.online, activity=discord.Game(current_state)
    )
